...
void Neuron::updateInputWeights(Layer &prevLayer){
  ...
  for (unsigned n = 0; n < prevLayer.size(); n++){
    ...
    double newDeltaWeight =
            ...
            eta // overall net learning rate (0.0 slow learner, 0.2 mediumd learner, 1.0 reckless learner)
            * neuron.getOutputVal()
            * m_gradient
            // Also add momentum = a fraction of the previous delta weight
            * alpha // momentum (0.0 no momentum, 0.5 moderate momentum(last weight change))
            * oldDeltaWeight;
